...
from app.services.notification_service import create_notification, process_notification
...
def mark_doctor_leave(
    ...
    # -----------------------------------------------------
    # CANCEL APPOINTMENTS & COMMIT DB TRANSACTION
    # -----------------------------------------------------
    ...
    # -----------------------------------------------------
    # CREATE AND PROCESS PATIENT NOTIFICATIONS
    # -----------------------------------------------------
    ...
    for appointment in affected_appointments:
        ...
        if patient and patient.email:
            ...
            body_text = (
                f"Your appointment with the doctor on {appt_time_str} "
            ...
            process_notification(db, patient_notification)
            patient_notification_count += 1

    # -----------------------------------------------------
    # CREATE AND PROCESS DOCTOR NOTIFICATION
    # -----------------------------------------------------
    ...
    if doctor_user and doctor_user.email:
        doctor_notification = create_notification(
            ...
            body=(
                f"You have been marked as on leave for {leave_date}."
                + (f" Reason: {reason}" if reason else "")
                + f"\n\nAffected appointments cancelled: {len(affected_appointments)}"
            )
        )

        process_notification(db, doctor_notification)

    return {
        "message": "Doctor marked on leave successfully",
        "doctor_id": doctor_id,
        "leave_date": leave_date,
        "affected_appointments": len(affected_appointments),
